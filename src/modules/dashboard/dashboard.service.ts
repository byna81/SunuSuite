...
export class DashboardService {
  ...
  async getAccountingDashboard(tenantId: string) {
    ...
    const saleDashboard = dashboards[0] || {};
    const rentalDashboard = dashboards[1] || {};
    const yangoDashboard = dashboards[2] || {};

    const saleRevenue = this.toNumber(
      saleDashboard?.revenues?.totalSalesRevenue,
    );
    const saleExpenses = this.toNumber(
      saleDashboard?.expenses?.totalExpenses,
    );
    const saleNetResult = this.toNumber(
      saleDashboard?.finance?.netResult,
    );
    const saleOutstanding = this.toNumber(
      saleDashboard?.finance?.totalOutstandingSales,
    );

    const rentalRevenue = this.toNumber(
      rentalDashboard?.revenues?.totalRentalRevenue,
    );
    const rentalExpenses = this.toNumber(
      rentalDashboard?.expenses?.totalExpenses,
    );
    const rentalNetResult = this.toNumber(
      rentalDashboard?.finance?.netResult,
    );
    const rentalOutstanding = this.toNumber(
      rentalDashboard?.finance?.totalOutstandingRentals,
    );

    const yangoRevenue = this.toNumber(
      yangoDashboard?.revenues?.totalDriverPayments,
    );
    const yangoExpenses = this.toNumber(
      yangoDashboard?.expenses?.totalExpenses,
    );
    const yangoOwnerPaid = this.toNumber(
      yangoDashboard?.ownerSettlements?.totalOwnerSettlementsPaid,
    );
    const yangoOwnerRemaining = this.toNumber(
      yangoDashboard?.ownerSettlements?.totalOwnerSettlementsRemaining,
    );
    const yangoNetResult = this.toNumber(
      yangoDashboard?.finance?.netResult,
    );
    const yangoLatePayments = this.toNumber(
      yangoDashboard?.finance?.totalLatePaymentsAmount,
    );
    ...
    const globalOutstanding =
      saleOutstanding + rentalOutstanding + yangoLatePayments;
